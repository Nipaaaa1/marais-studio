<template>
  <Section id="works">
    <SectionTitle name="03 - Works">
      <span class="text-primary">Collaborating</span> with Purposeful Tech
      Visionaries
    </SectionTitle>
    <div class="grid grid-cols-1 gap-3 md:grid-cols-2 md:gap-x-5 md:gap-y-6">
      <Cards title="wave">
        <img
          src="/public/images/mobile/wave.png"
          alt="wave's Website"
          class="h-max w-full md:hidden"
        />
        <img
          src="/public/images/desktop/wave.png"
          alt="wave's Website"
          class="hidden h-max w-full md:block"
        />
      </Cards>
      <Cards title="reflecta">
        <img
          src="/public/images/mobile/reflecta.png"
          alt="reflecta's Website"
          class="h-max w-full md:hidden"
        />
        <img
          src="/public/images/desktop/reflecta.png"
          alt="reflecta's Website"
          class="hidden h-max w-full md:block"
        />
      </Cards>
      <Cards title="solo">
        <img
          src="/public/images/mobile/solo.png"
          alt="solo's Website"
          class="h-max w-full md:hidden"
        />
        <img
          src="/public/images/desktop/solo.png"
          alt="solo's Website"
          class="hidden h-max w-full md:block"
        />
      </Cards>
      <Cards title="dappr">
        <img
          src="/public/images/mobile/dappr.png"
          alt="dappr's Website"
          class="h-max w-full md:hidden"
        />
        <img
          src="/public/images/desktop/dappr.png"
          alt="dappr's Website"
          class="hidden h-max w-full md:block"
        />
      </Cards>
    </div>
  </Section>
</template>
